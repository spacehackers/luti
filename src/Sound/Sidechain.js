...
const Sidechain = (props) => {
  ...
  useEffect(() => {
    ...
    sidechainInputNode.current = props.audioContext.createWaveShaper();
    const curve = new Float32Array(65536);
    for (let i = -32768; i < 32768; i++)
      curve[i + 32768] = -((i > 0 ? i : -i) / 32768);
    sidechainInputNode.current.curve = curve;

    followerNode.current = props.audioContext.createBiquadFilter();
    followerNode.current.type = "lowpass";

    sidechainInputNode.current.connect(followerNode.current);
